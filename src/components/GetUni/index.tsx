...
const GetUni = ({user}: { user: string }) => {
  ...
  const [univ2TSDBalance, setUniv2Balance] = useState(initState);
  ...
  useEffect(() => {
    ...
    async function updateUserInfo() {
      ...
      if (!isCancelled) {
        ...
        setUniv2Balance(new BigNumber(toTokenUnitsBN(uniTSDBalanceStr, UNI.decimals)));

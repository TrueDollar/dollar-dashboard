  ...
  useEffect(() => {
    ...
    async function updateUserInfo() {
      const [
        ...
        getRecordedVote(TSDS.addr, user, candidate),
      ...
      if (!isCancelled) {
        ...
        setUserStake(toTokenUnitsBN(userStakeStr, TSDS.decimals));
      }
    }
    updateUserInfo();
  ...
  useEffect(() => {
    ...
    async function updateUserInfo() {
      let [
        ...
        getApproveFor(TSDS.addr, candidate),
        getRejectFor(TSDS.addr, candidate),
        getTokenTotalSupply(TSDS.addr),
        getEpoch(TSDS.addr),
        getStartFor(TSDS.addr, candidate),
        getPeriodFor(TSDS.addr, candidate),
        getIsInitialized(TSDS.addr, candidate),
      ]);
      ...
      if (epochN > endsAfter) {
        totalStakeStr = await getTotalBondedAt(TSDS.addr, endsAfter);
      }

      if (!isCancelled) {
        setApproveFor(toTokenUnitsBN(approveForStr, TSDS.decimals));
        setRejectFor(toTokenUnitsBN(rejectForStr, TSDS.decimals));
        setTotalStake(toTokenUnitsBN(totalStakeStr, TSDS.decimals));
        setEpoch(epochN);

...
function PurchaseHistory({
  ...
  useEffect(() => {
    ...
    async function updateUserInfo() {
      const epochsFromEvents = await getCouponEpochs(TSDS.addr, user);
      const epochNumbers = epochsFromEvents.map(e => parseInt(e.epoch));
      const balanceOfCoupons = await getBatchBalanceOfCoupons(TSDS.addr, user, epochNumbers);
      const couponsExpirations = await getBatchCouponsExpiration(TSDS.addr, epochNumbers);
  ...
  return (
    <DataView
      ...
      renderEntry={(epoch) => [
        ...
        formatBN(toTokenUnitsBN(epoch.coupons, TSD.decimals), 2),
        formatBN(toTokenUnitsBN(epoch.balance, TSD.decimals), 2),
        ...
        <Button
          ...
          onClick={() => redeemCoupons(
            TSDS.addr,
            epoch.epoch,
            epoch.balance.isGreaterThan(toBaseUnitBN(totalRedeemable, TSD.decimals))
              ? toBaseUnitBN(totalRedeemable, TSD.decimals)
              : epoch.balance
          )}
